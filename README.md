...
- **Song.** `olafs-song.mp3` starts by itself where the browser allows it. Most browsers, especially on phones, block sound until the visitor touches the page. In that case the play button pulses gently and the song starts with the first tap, click or key press anywhere on the page. This is browser policy and cannot be bypassed. The pill button at the bottom left pauses and resumes it. If the visitor pauses, nothing restarts it automatically.
